...
public class SpotMapController {
    ...
    @GetMapping("/{stadiumId}/{category}/{contentId}")
    @Operation(summary = "주변 관광지 마킹 클릭", description = "주변 관광지 마커를 클릭 시 상세 내용을 출력합니다.")
    public ResponseEntity<SpotDetailInfoDto> getNearSpotDetailInfo(@PathVariable Long stadiumId , @PathVariable String category, @PathVariable Long contentId, @CurrentUser UserPrincipal userPrincipal){
