...
public class SpotStadiumController {
    ...
    public ResponseEntity<List<SpotMapResponseDto>> getStadiumNearSpot(@PathVariable String stadium, @PathVariable String category, @PathVariable int radius,
                                                       @PathVariable double nowX, @PathVariable double nowY) throws IOException {
        List<SpotMapResponseDto> nearSpot = spotService.getNearSpot(nowX, nowY, stadium, category, radius);
